...
function initPageLoader() {
  ...
  function hideLoader() {
    ...
    //    We use BOTH transitionend AND a setTimeout fallback —
    //    whichever fires first wins. This guarantees removal even
    //    if the CSS transition never fires (reduced-motion, etc.)
    const CSS_TRANSITION_MS = 600; // slightly longer than .55s
  ...
  if (document.readyState === "complete") {
    // Page already fully loaded when script ran
    tryHide();
  } else {
    // Wait for window.load (images, fonts, etc.)
    window.addEventListener("load", tryHide, { once: true });

    // Absolute safety net — hides loader no matter what
...
function initNavbar() {
  ...
  const onScroll = throttle(() => {
    ...
    navLinks.forEach((link) => {
      ...
      // Normalise href to extract the target section id
      // Handles: "#about"  "/#about"  "/menu.html" (active when on that page)
      let targetSection = "";

      if (href.startsWith("#")) {
        targetSection = href.slice(1); // "#about" → "about"
      } else if (href.startsWith("/#")) {
        targetSection = href.slice(2); // "/#about" → "about"
      }

      // Special case: "menu.html" or "/menu.html" link is active when on menu page
      const isMenuLink =
        href === "menu.html" ||
        href === "/menu.html" ||
...
function initSmoothScroll() {
  ...
  document.addEventListener("click", (e) => {
    ...
    // ── Wipe hash from URL after scrolling ──────────────────
    // history.replaceState() updates the address bar without
    // triggering a page reload or adding a browser history entry.
    // The user sees domain.com — nothing after it.
    history.replaceState(null, "", window.location.pathname);
  });

  // ── Also intercept cross-page /#section links (from menu.html) ──
  // When the user navigates to /#about, the page loads with a hash.
  // We scroll to the target then immediately clean the URL.
  document.addEventListener("click", (e) => {
    const link = e.target.closest('a[href^="/#"]');
    if (!link) return;

    const href = link.getAttribute("href");
    const sectionId = href.replace("/", ""); // "/#about" → "#about"
    const isOnHomePage =
    ...
    if (isOnHomePage) {
      // Already on home page — scroll without navigating
      e.preventDefault();
    ...
    // If not on home page, let the browser navigate naturally to /#section
    // then handleArrivalHash() below will clean it up on the next page load
  });
}

/* ═══════════════════════════════════════════════════════════
   3b. ARRIVAL HASH HANDLER
   When a user arrives at domain.com/#about (e.g. from menu.html),
   we scroll to that section smoothly then replace the URL with
   domain.com — no hash visible.
═══════════════════════════════════════════════════════════ */
function handleArrivalHash() {
  ...
  if (!target) {
    // Section not found — just clean the URL silently
    history.replaceState(null, "", window.location.pathname);
    return;
  }
  ...
  // Delay slightly so the page has fully rendered before scrolling
  setTimeout(() => {
    const top = target.getBoundingClientRect().top + window.scrollY - offset;
    window.scrollTo({ top, behavior: "smooth" });
  ...
  function applyMenuFilters() {
    ...
    menuCards.forEach((card) => {
      ...
      if (match) {
        ...
        // Use a data attribute for stagger delay — avoids inline animation
        // string with CSS variables which don't resolve in inline styles
        card.style.setProperty(
          "--card-delay",
          `${(visibleCount % 8) * 0.055}s`,
        );
        card.classList.remove("menu-card--animate");
        // requestAnimationFrame ensures the class removal is painted first,
        // then we add it back to restart the animation cleanly
...
function handleFormSubmit(e) {
  ...
  // ── Format date: 2024-12-25 → Wednesday, 25 December 2024 ──
...
function createOrderLink(itemName) {
  // Decode HTML entities before URL-encoding for WhatsApp
